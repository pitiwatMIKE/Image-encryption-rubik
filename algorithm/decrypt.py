...
def decryptionImage(pathfile):
	...
	# read file key
	filekey = open("keys.txt", 'r')
	dataKeys = filekey.read().split('\n')
	filekey.close()

	arrKr = dataKeys[dataKeys.index("Vector Kr : ") + 1: dataKeys.index("Vector Kc : ")]
	arrKc = dataKeys[dataKeys.index("Vector Kc : ") + 1: dataKeys.index("ITER_MAX : ")]
	arrITER_MAX = dataKeys[dataKeys.index("ITER_MAX : ") + 1: -1]
	# end read file key
	...
	for i in range(m):
		Kr.append(int(arrKr[i]))

	print('Enter value of Kc')
	for i in range(n):
		Kc.append(int(arrKc[i]))

	print('Enter value of ITER_MAX')
	ITER_MAX = int(arrITER_MAX[0])
